...
#define INST_EX(p, opcode, v1, v2, v3) \
...
#define zvm_raise(vm, t, c) \
    ...
    (vm)->exception_metadata.exception_type = ZVM_EXCEPT_TYPE_ ## t;     \
    (vm)->exception_metadata.exception_code = ZVM_EXCEPT_CODE_ ## c;

#define zvm_exception_get_type(vm)  (vm)->exception_metadata.exception_type
#define zvm_exception_get_code(vm)  (vm)->exception_metadata.exception_code

#define zvm_exception_reset(vm)      \
    (vm)->has_exception  = false;    \
    (vm)->exception_metadata.exception_type = -1;       \
    (vm)->exception_metadata.exception_code = -1;
